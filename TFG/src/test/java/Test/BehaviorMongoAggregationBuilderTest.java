...
public class BehaviorMongoAggregationBuilderTest {
	...
	public void test() {
		 ...
		 * ="mongodb://"+USER+":"+PASS+"@127.0.0.1:27017/"+DATABASENAME ; MongoLogic
		 * connectionDatabase = null ; connectionDatabase = new
		 * MongoLogic(connectionString); AggregationQueryBuilders aggregates = new
		 * AggregationQueryBuilders(); BehaviorMatchStagebuilder(connectionDatabase,
		 * aggregates); aggregates.clearPipeline();
	...
	public void BehaviorMatchStagebuilder(MongoLogic connectionDatabase, AggregationQueryBuilders aggregates) {
		...
		insertFielValuePairTest(); // se intertan los datos de test en los vectores auxiliares para crear el objeto
									// MatfilterObject
		...
		AggregateIterable<Document> iterable = connectionDatabase.getAnyCollection(DATABASENAME, COLLECTIONAME)
				.aggregate(aggregates.getPipeline());
	...
	public void insertFielValuePairTest() {
		...
		/*
		 * makeTestArgumentsFields("asdd","asdd"); makeTestArguments("nad");
		 * MatchFilterObject auxTest = new
		 * MatchFilterObject(fieldName,fieldValue,filters); Assert.assertEquals(3,
		 * auxTest.getFieldValueFilters().size());
		 */
	}
