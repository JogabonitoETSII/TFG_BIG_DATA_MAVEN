...
public void  MongoLogicConnecitonTest(MongoLogic connectionDatabase) {
	...
	/**
	 * Singles stage aggregation.
	 * Test realizado con la finalidad de comprobar si conectamos y somos capaces de leer de la BBDD
	 * @param connectionDatabase the connection database
	 */
	public void singlesStageAggregation(MongoLogic connectionDatabase) {
		...
		AggregateIterable<Document> iterable =  connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAME).aggregate(pipeline);
	...
	public void testInsertMany(MongoLogic connectionDatabase) {
		...
		insertTestDOcument =  connectionDatabase.insertManyDocuments(connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS),getInsertManyTest());
		for(Document obtainObjecId : insertTestDOcument) {
			objectIdTest.add(obtainObjecId.getObjectId("_id"));
		}
		Long expected = (long) 3;
		Long inserted = connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS).countDocuments(Filters.in("_id",objectIdTest));
	...
	public void testInsertOne(MongoLogic connectionDatabase) {
		...
		insertTestDOcument = connectionDatabase.insertOneDocument(connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS) , getInsertTest());
		Document inserted = connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS).find(Filters.eq("_id",
				insertTestDOcument.getObjectId("_id"))).first();
	...
	void testUpdateOne(MongoLogic connectionDatabase) {
	    ...
	    connectionDatabase.updateOne(connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS),documentToUbdate ,getInsertTest() );
		// crear un objeto idOBject y pasarlo por parametro como se penso originalmente.
	    
	    
	
	    
	    updatdeDocument = connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS).find(Filters.eq("_id",documentToUbdate)).iterator().tryNext();
	...
	void testUpdateMany(MongoLogic connectionDatabase) {
		...
		iteratorFInd = connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS).find(updatdeDocument); // guardamos el objectId Para confirmar el update 
		listObjectUpdate.add(iteratorFInd.iterator().tryNext().getObjectId("_id")); // guardamos el object ID
		connectionDatabase.updateMany(connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS),documentToUpdate ,getInsertTest() ); // realizamos el update
		iteratorFInd = connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS).find(Filters.eq(listObjectUpdate.get(0))); // volvemos a coger el documento actualizado
	...
	public void deleteOneTest(MongoLogic connectionDatabase) {
		...
		ObjectId documentToUbdate = new ObjectId("5f2922eb000de42e76ce8696");
	    connectionDatabase.deleteOne(connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS),documentToUbdate);
	    Assert.assertNull(connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS).find(Filters.eq(documentToUbdate)).iterator().tryNext() );
	}
	...
	public void deleteManyTest(MongoLogic connectionDatabase) {
		...
		documentToUpdate.put("cuidad" , "la laguna"); // se meten los campos en un documento para utilziar de fomra nativa la api de Mongo
		...
		Long documentsToDelete  = connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS).countDocuments(updatdeDocument);
		
		
		resultToDelete = connectionDatabase.deleteMany(connectionDatabase.getAnyCollection(DATABASENAME,COLLECTIONAMECRUDS),documentToUpdate);
	...
	public void createDocumentsToManyInsertUpdateTest() {
		...
		//System.out.println("contenido del arraylist al crearlo  0   " + getInsertManyTest() );
		getInsertTest().put("cuidad", "la laguna" );
		getInsertTest().put("codigo postal", 38025);
		getInsertTest().put("Direccion", "calle barcelona nº6" );
		getInsertTest().put("Piso", 3 );
		...
		//System.out.println("contenido del arraylist al crearlo  1  " + getInsertManyTest() );
		...
		setInsertTest(new HashMap<String, Object>());
		getInsertTest().clear();
		//System.out.println("contenido del arraylist al crearlo 2  " + getInsertManyTest() );
		...
		getInsertManyTest().add(getInsertTest());
		//getInsertTest().clear();
		//System.out.println("contenido del arraylist al crearlo  3 " + getInsertManyTest() );
